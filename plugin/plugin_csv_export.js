...
(function () {
  ...
  var DICT = {
    ja: {
      ...
      colsLine1:     '出力列: プロジェクト / # / タイトル / ステータス / 担当者',
      colsLine2:     'マイルストーン / 開始日 / 期限 / 完了日 / SP / ラベル',
      colsLine3:     'チェックリスト進捗 / 作成日',
      btnClose:      '閉じる',
      ...
      csvHeader:     ['プロジェクト','#','タイトル','ステータス','担当者','マイルストーン','開始日','期限','完了日','SP','ラベル','チェックリスト進捗','作成日'],
    },
    en: {
      ...
      colsLine1:     'Columns: Project / # / Title / Status / Assignee',
      colsLine2:     'Milestone / Start / Due / Completed / SP / Labels',
      colsLine3:     'Checklist progress / Created',
      btnClose:      'Close',
      ...
      csvHeader:     ['Project','#','Title','Status','Assignee','Milestone','Start','Due','Completed','SP','Labels','Checklist','Created'],
    },
  };
  ...
  function buildCsv(project) {
    ...
    var rows = project.tasks.map(function(task) {
      var clTotal = (task.checklist || []).length;
      var clDone  = (task.checklist || []).filter(function(c) { return c.done; }).length;
      var clStr   = clTotal > 0 ? (clDone + '/' + clTotal) : '';
      return [
        project.name, '#' + task.number, task.title,
  ...
  function buildModal() {
    ...
    el.innerHTML =
      '<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:28px 32px;width:360px;max-width:96vw">' +
